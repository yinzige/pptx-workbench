...
if (packageJson.version !== "1.6.11.1") {
  throw new Error(`package.json version must be 1.6.11.1 for v1.6.11.1; got ${packageJson.version ?? "missing"}`);
}
assertContains(await fs.readFile(resolveFromProject("package.json"), "utf8"), "bridge:register-current", "v1.6.11.1 current thread register script");
assertContains(await fs.readFile(resolveFromProject("package.json"), "utf8"), "bridge:create-token", "v1.6.11.1 bridge token script");
assertContains(await fs.readFile(resolveFromProject("package.json"), "utf8"), "bridge:probe-app-server", "v1.6.11.1 app-server probe script");

const specSource = await fs.readFile(defaultSpecPath, "utf8");
assertContains(spec.title, "v1.6.11.1", "spec v1.6.11.1 title");
...
assertContains(previewParity, "Version: v1.6.11.1", "preview parity version");
...
assertContains(serverSource, "/api/upload-reference", "v1.6.11.1 upload reference API route");
assertContains(serverSource, "/api/upload-reference/metadata", "v1.6.11.1 large upload metadata API route");
assertContains(serverSource, 'limit: "50mb"', "v1.6.11.1 upload raw route stays under 50mb");
assertContains(serverSource, "/api/codex-queue/process", "v1.6.11.1 Codex queue API route");
assertContains(serverSource, "appendUploadIntent", "v1.6.11.1 upload intent event writer");
assertContains(serverSource, "uploadIntent", "v1.6.11.1 workbench state upload intent path");
assertContains(serverSource, "/api/undo", "v1.6.11.1 undo API route");
assertContains(serverSource, "/api/redo", "v1.6.11.1 redo API route");
...
assertContains(serverSource, "/api/codex-bridge", "v1.6.11.1 Codex bridge API route");
assertContains(serverSource, "/api/codex-bridge/connect-token", "v1.6.11.1 Codex bridge token connect route");
assertContains(serverSource, "appendCodexBridgeEvent", "v1.6.11.1 unified bridge event writer");
assertContains(serverSource, "codexBridge,", "v1.6.11.1 workbench state Codex bridge summary");
...
assertContains(serverSource, "writesDisk: true", "v1.6.11.1 playback QA writes disk");
assertContains(serverSource, "designSystem:", "v1.6.11.1 workbench state design system summary");
assertContains(serverSource, "lastExportMatchesProject", "v1.6.11.1 export history match status");
assertContains(serverSource, "historical-export", "v1.6.11.1 historical export status");
assertContains(serverSource, "syncRevisionPlanProject", "v1.6.11.1 revision plan project sync");
assertContains(serverSource, "generateDecks(spec)", "v1.6.11.1 export regenerates before copy");
assertContains(exportManagerSource, 'exportRootDir = "/Users/bruce/Desktop/PPT"', "locked export root");
assertContains(exportManagerSource, 'defaultProjectName = "测试-v1.6.11.1"', "default v1.6 project name");
...
assertContains(uploadRegistrySource, "large-file-audit", "v1.6.11.1 large file audit mode");
assertContains(uploadRegistrySource, "50 * 1024 * 1024", "v1.6.11.1 upload split threshold");
assertContains(undoManagerSource, "const maxUndo = 100", "v1.6.11.1 max undo stack");
assertContains(undoManagerSource, "redoStackDir", "v1.6.11.1 redo stack");
assertContains(undoManagerSource, "createUndoSnapshotInternal", "v1.6.11.1 redo does not clear redo stack before restore");
assertContains(codexQueueSource, "createUndoSnapshot", "v1.6.11.1 Codex queue creates undo snapshot");
assertContains(codexQueueSource, "applyDeterministicEdit", "v1.6.11.1 Codex queue deterministic edits");
assertContains(codexQueueSource, "删除句号", "v1.6.11.1 Codex queue supports period deletion");
assertContains(codexQueueSource, "needs-codex", "v1.6.11.1 Codex queue does not fake design edits");
assertContains(codexQueueSource, "afterSpec === beforeSpec", "v1.6.11.1 Codex queue rejects empty diff");
assertNotContains(codexQueueSource, "codexNotes", "v1.6.11.1 Codex queue must not fake process by notes only");
...
assertContains(revisionPlanSource, "objectId", "v1.6.11.1 revision action objectId");
assertContains(revisionPlanSource, "objectRole", "v1.6.11.1 revision action objectRole");
...
assertContains(codexBridgeSource, 'resolveFromProject("events", "codex-events.jsonl")', "v1.6.11.1 Codex bridge JSONL path");
assertContains(codexBridgeSource, 'resolveFromProject(".codex-bridge", "current-thread.json")', "v1.6.11.1 current thread config path");
assertContains(codexBridgeSource, 'resolveFromProject(".codex-bridge", "pending-tokens.jsonl")', "v1.6.11.1 pending bridge token path");
assertContains(codexBridgeSource, "createCodexBridgeToken", "v1.6.11.1 one-time bridge token creator");
assertContains(codexBridgeSource, "consumeCodexBridgeToken", "v1.6.11.1 one-time bridge token consumer");
assertContains(codexBridgeSource, "updateCodexBridgeAppServer", "v1.6.11.1 app-server probe writes current bridge status");
assertContains(codexBridgeSource, "expiresAt", "v1.6.11.1 bridge expiry field");
assertContains(codexBridgeSource, "used: true", "v1.6.11.1 token single-use marker");
assertContains(codexBridgeSource, "detectAppServer", "v1.6.11.1 app-server detection");
assertContains(codexBridgeSource, "CODEX_BRIDGE_ALLOW_MOCK", "v1.6.11.1 mock app-server requires explicit opt-in");
assertContains(codexBridgeSource, "不把普通 HTTP endpoint 当作真实 app-server", "v1.6.11.1 does not treat guessed HTTP endpoint as real app-server");
assertContains(codexAppServerClientSource, "JSON-RPC", "v1.6.11.1 app-server JSON-RPC client");
assertContains(codexAppServerClientSource, "codex app-server --stdio", "v1.6.11.1 stdio app-server transport");
assertContains(codexAppServerClientSource, "thread/resume", "v1.6.11.1 thread resume method");
assertContains(codexAppServerClientSource, "turn/start", "v1.6.11.1 turn start method");
assertContains(codexAppServerClientSource, "text_elements: []", "v1.6.11.1 turn/start matches generated schema text input");
assertContains(codexAppServerClientSource, "outputs\", \"codex-app-server-probe.md", "v1.6.11.1 app-server probe report path");
assertContains(codexAppServerClientSource, "experimental / unsupported", "v1.6.11.1 websocket limitation note");
assertContains(codexProbeScriptSource, "probeCodexAppServer", "v1.6.11.1 probe script");
assertContains(codexProbeScriptSource, "updateCodexBridgeAppServer", "v1.6.11.1 probe updates UI bridge status");
assertContains(codexBridgeScriptSource, "sendWorkbenchEventToCodexAppServer", "v1.6.11.1 bridge uses JSON-RPC app-server client");
assertContains(codexBridgeScriptSource, "updateCodexBridgeAppServer", "v1.6.11.1 bridge updates UI bridge status");
assertContains(codexBridgeScriptSource, "targetThreadId", "v1.6.11.1 bridge records target thread");
assertContains(codexBridgeScriptSource, "sentAt", "v1.6.11.1 bridge records sentAt");
assertContains(codexBridgeScriptSource, "attemptCount", "v1.6.11.1 bridge records attempt count");
assertContains(codexBridgeSource, "bridge_unavailable", "v1.6.11.1 bridge unavailable status");
assertContains(codexBridgeSource, "waiting_codex", "v1.6.11.1 waiting Codex status");
assertContains(codexBridgeSource, "annotation_submitted", "v1.6.11.1 annotation bridge event type");
assertContains(codexBridgeSource, "upload_reference", "v1.6.11.1 upload bridge event type");
...
assertContains(clientSource, 'id="upload-reference-trigger"', "v1.6.11.1 top-level upload action");
assertContains(clientSource, ">上传</button>", "v1.6.11.1 upload button label");
assertNotContains(clientSource, 'id="generate"', "v1.6.11.1 removes top-level generate button");
...
assertContains(clientSource, "codex-bridge-status", "v1.6.11.1 top Codex bridge status");
assertContains(clientSource, "未连接 Codex", "v1.6.11.1 disconnected Codex status copy");
assertContains(clientSource, "已连接 Codex", "v1.6.11.1 connected Codex status copy");
assertContains(clientSource, "Codex 桥接不可用，事件已排队", "v1.6.11.1 bridge unavailable queued copy");
assertContains(clientSource, "连接 token 已失效，请重新连接", "v1.6.11.1 expired token status copy");
assertContains(clientSource, "已注册会话，未发现 app-server", "v1.6.11.1 app-server unavailable copy");
assertContains(clientSource, "/api/codex-bridge/connect-token", "v1.6.11.1 frontend consumes bridge token");
...
if ((clientSource.match(/id="playback-top"/g) ?? []).length !== 1) {
  throw new Error("v1.6.11.1 must expose exactly one top-level playback entry");
}
assertContains(clientSource, "/api/export", "desktop export frontend call");
assertContains(clientSource, "项目名", "project name UI");
assertContains(clientSource, "测试-v1.6.11.1", "default project name UI");
...
assertContains(clientSource, "bridge 队列", "v1.6.11.1 annotation writes bridge queue copy");
...
assertContains(clientSource, "type AnnotationMode", "v1.6.11.1 annotation mode type");
assertContains(clientSource, "annotationMode", "v1.6.11.1 annotation mode state");
assertContains(clientSource, "annotation-mode-toggle", "v1.6.11.1 annotation secondary mode button");
assertContains(clientSource, "自由批注", "v1.6.11.1 free annotation mode label");
...
assertContains(clientSource, "/api/codex-queue/process", "v1.6.11.1 Codex queue process API");
assertNotContains(clientSource, "让 Codex 处理", "v1.6.11.1 annotation processing is automatic");
assertContains(clientSource, "await processCodexQueue()", "v1.6.11.1 annotation submit auto-processes queue");
assertContains(clientSource, "需要 Codex 处理", "v1.6.11.1 design edits are not fake processed");
assertContains(clientSource, "等待 Codex 在对话中确认用途", "v1.6.11.1 upload waits for Codex conversation");
assertNotContains(clientSource, "已写入 events/upload-intent.jsonl；等待用户选择", "v1.6.11.1 upload event path is not main UI copy");
assertContains(clientSource, "撤销 0 / 100", "v1.6.11.1 always-visible undo action");
assertContains(clientSource, "返回 0", "v1.6.11.1 always-visible redo action");
assertContains(clientSource, "/api/undo", "v1.6.11.1 undo API frontend call");
assertContains(clientSource, "/api/redo", "v1.6.11.1 redo API frontend call");
assertContains(clientSource, "/api/upload-reference/metadata", "v1.6.11.1 large upload metadata route");
assertContains(clientSource, "largePptxThresholdBytes", "v1.6.11.1 frontend upload size split");
...
assertNotContains(clientSource, "导入 PPTX 审计", "v1.6.11.1 UI cleanup removes old audit menu action");
assertNotContains(clientSource, "上传 PPTX 审计", "v1.6.11.1 UI cleanup removes old upload audit copy");
assertContains(clientSource, '<div class="toolbar-right">', "v1.6.11.1 right toolbar contains annotation and history actions");
assertContains(clientSource, 'id="undo-action"', "v1.6.11.1 undo action remains visible");
assertContains(clientSource, 'id="redo-action"', "v1.6.11.1 redo action remains visible");
...
assertContains(clientSource, "交付摘要", "v1.6.11.1 delivery summary card");
...
assertNotContains(clientSource, "Codex 上下文", "v1.6.11.1 hides copyable Codex context from main flow");
assertNotContains(clientSource, "复制提示词", "v1.6.11.1 hides copy prompt from main flow");
...
assertContains(clientSource, "WPS 状态页映射", "v1.6.11.1 WPS state mapping field");
...
assertContains(clientSource, "预览设置", "v1.6.11.1 preview states moved into settings");
...
assertContains(clientSource, "WPS 状态页映射", "v1.6.11.1 WPS state mapping in motion panel");
...
assertContains(clientSource, "playbackAnimating", "v1.6.11.1 playback blocks clicks during animation");
assertContains(clientSource, "playPlaybackPreflightAnimations", "v1.6.11.1 playback preflight animation player");
assertContains(clientSource, ".finished", "v1.6.11.1 playback waits for animation finish promise");
...
assertContains(clientSource, "click_overrun", "v1.6.11.1 click overrun field");
assertContains(clientSource, "invisible_change_risk", "v1.6.11.1 invisible change risk field");
assertContains(clientSource, "event_type", "v1.6.11.1 playback QA event type");
assertContains(clientSource, "scene-beat-click", "v1.6.11.1 playback QA scene beat event");
assertContains(clientSource, "slide-transition", "v1.6.11.1 playback QA slide transition event");
assertContains(clientSource, "coveredSlides", "v1.6.11.1 playback QA covered slides field");
assertContains(clientSource, "expectedSlides", "v1.6.11.1 playback QA expected slides field");
assertContains(clientSource, "sceneBeatClicks", "v1.6.11.1 playback QA sceneBeatClicks field");
assertContains(clientSource, "manualExtraClicks", "v1.6.11.1 playback QA manualExtraClicks field");
assertContains(clientSource, "仅覆盖部分页面", "v1.6.11.1 playback partial coverage conclusion");
assertContains(clientSource, "不能判定“点击逻辑正确”", "v1.6.11.1 playback no false-positive conclusion");
assertContains(clientSource, "persistPlaybackQaSession", "v1.6.11.1 playback QA persistence");
assertContains(clientSource, "/api/playback-qa/session", "v1.6.11.1 playback QA session frontend POST");
assertContains(clientSource, "/api/playback-qa", "v1.6.11.1 playback QA frontend GET");
assertContains(clientSource, "playbackLastPersistStatus", "v1.6.11.1 playback QA persisted status");
assertContains(clientSource, "历史导出", "v1.6.11.1 historical export UI label");
assertContains(clientSource, "objectId", "v1.6.11.1 object-level revision field");
assertContains(clientSource, "objectRole", "v1.6.11.1 object-level revision role");
assertContains(clientSource, "playback-click", "v1.6.11.1 QA log playback-click category");
assertContains(clientSource, "playback-session", "v1.6.11.1 QA log playback-session category");
...
assertContains(clientSource, "renderSpecElement", "v1.6.11.1 same-source deck-spec element renderer");
assertContains(clientSource, "orderedSlideElements", "v1.6.11.1 same-source layer traversal");
assertContains(clientSource, "elementStyle", "v1.6.11.1 element geometry/style mapper");
assertContains(clientSource, "13.333", "v1.6.11.1 coordinate mapping width basis");
assertContains(clientSource, "7.5", "v1.6.11.1 coordinate mapping height basis");
assertContains(clientSource, "more-actions-popover", "v1.6.11.1 more actions popover");
assertContains(clientSource, "preview-settings-popover", "v1.6.11.1 preview settings popover");
assertContains(clientSource, "timeline-modal", "v1.6.11.1 full timeline modal");
assertContains(clientSource, "查看时间轴", "v1.6.11.1 timeline secondary window action");
assertNotContains(clientSource, "slide-stage-title", "v1.6.11.1 old hard-coded preview title");
assertNotContains(clientSource, "slide-body-copy", "v1.6.11.1 old hard-coded preview body");
assertNotContains(clientSource, "slide-info-layer", "v1.6.11.1 old hard-coded reveal layer");
...
assertContains(clientSource, "shouldShowElement", "v1.6.11.1 hidden reveal control");
assertContains(clientSource, "shouldShowElement", "v1.6.11.1 revealed element control");
...
assertContains(styleSource, "grid-template-columns: 34px minmax(0, 1fr) 36px", "v1.6.11.1 both collapsed rails keep canvas priority");
assertContains(styleSource, "grid-template-columns: 210px minmax(0, 1fr) 36px", "v1.6.11.1 collapsed right keeps restore rail");
assertContains(styleSource, "overflow-wrap: anywhere", "v1.4.3 right panel overflow wrap");
assertContains(styleSource, "word-break: normal", "v1.6.11.1 avoids single-character vertical wrapping");
...
assertContains(styleSource, "v1.6.11.1 Codex-style same-source preview", "v1.6 CSS marker");
...
assertContains(styleSource, "v1.6.11.1 production acceptance layout", "v1.6.11.1 layout CSS marker");
assertContains(styleSource, "fit-to-available-area", "v1.6.11.1 fit strategy marker");
assertContains(styleSource, ".floating-popover[data-anchor-ready=\"true\"]", "v1.6.11.1 anchored popover CSS");
assertContains(styleSource, "v1.6.11.1 Codex-native PPTX production foundation", "v1.6.11.1 production foundation CSS");
assertContains(styleSource, ".codex-toolbar", "v1.6.11.1 compact top toolbar CSS");
assertContains(styleSource, ".annotation-button", "v1.6.11.1 annotation button CSS");
assertContains(styleSource, ".annotation-mode-toggle", "v1.6.11.1 annotation mode toggle CSS");
assertContains(styleSource, ".annotation-region-mode", "v1.6.11.1 free annotation cursor CSS");
assertContains(styleSource, "data:image/svg+xml", "v1.6.11.1 free annotation custom cursor");
assertContains(styleSource, ".annotation-selection", "v1.6.11.1 selection CSS");
assertContains(styleSource, ".annotation-ask-codex", "v1.6.11.1 Ask Codex capsule CSS");
assertContains(styleSource, ".annotation-drag-region", "v1.6.11.1 region drag CSS");
assertContains(styleSource, ".annotation-composer", "v1.6.11.1 annotation composer CSS");
assertContains(styleSource, "width: clamp(300px, 24cqw, 460px)", "v1.6.11.1 annotation composer width");
assertContains(styleSource, ".annotation-composer #annotation-submit:disabled", "v1.6.11.1 annotation submit empty state");
assertContains(styleSource, ".annotation-composer #annotation-submit.is-ready", "v1.6.11.1 annotation submit ready state");
assertContains(styleSource, "max-height: 68px", "v1.6.11.1 long annotation text height cap");
assertContains(styleSource, ".annotation-composer-input.is-multiline #annotation-submit", "v1.6.11.1 multiline submit bottom-right alignment");
assertContains(styleSource, "scrollbar-color", "v1.6.11.1 long annotation text scroll affordance");
assertContains(styleSource, "v1.6.11.1 dark readability: left thumbnails", "v1.6.11.1 dark thumbnail text readability");
assertContains(styleSource, ".app-shell[data-theme-mode=\"dark\"] .slide-thumb:not(.active) .thumb-meta strong", "v1.6.11.1 dark inactive thumbnail title contrast");
assertContains(styleSource, ".app-shell[data-theme-mode=\"dark\"] .slide-thumb.active .thumb-meta strong", "v1.6.11.1 dark active thumbnail title contrast");
assertContains(styleSource, ".app-shell[data-theme-mode=\"dark\"] .slide-thumb.active .thumb-meta small", "v1.6.11.1 dark active thumbnail subtitle contrast");
assertContains(styleSource, ".app-shell[data-theme-mode=\"system\"] .slide-thumb:not(.active) .thumb-index", "v1.6.11.1 system dark inactive thumbnail index contrast");
assertContains(styleSource, ".app-shell[data-theme-mode=\"system\"] .slide-thumb.active .thumb-index", "v1.6.11.1 system dark active thumbnail index contrast");
assertContains(styleSource, ".project-name-panel #project-name-save:not(:disabled)", "v1.6.11.1 project name primary save button");
assertContains(styleSource, "place-items: center", "v1.6.11.1 centered segmented menu text");
assertContains(clientSource, "bindAnnotationComposerControls", "v1.6.11.1 annotation composer input binding");
assertContains(clientSource, "submit.disabled = !hasText", "v1.6.11.1 annotation submit button state");
assertContains(clientSource, "submit.textContent = isMultiline ? \"✓\" : \"↑\"", "v1.6.11.1 annotation submit single and multiline icon");
assertContains(clientSource, "shiftKey", "v1.6.11.1 annotation composer shift-enter multiline");
assertContains(clientSource, "annotationComposerLineCount", "v1.6.11.1 stable annotation composer line count");
assertContains(clientSource, "lineCount > 1", "v1.6.11.1 annotation mode avoids height flicker");
assertContains(clientSource, "positionAnnotationComposerWithinWorkspace", "v1.6.11.1 annotation composer workspace positioning");
assertContains(clientSource, "clampToRange", "v1.6.11.1 annotation composer avoids side panels");
assertContains(clientSource, "annotationPlacementDirections", "v1.6.11.1 annotation composer uses PPT-center placement");
assertContains(clientSource, "seededFraction", "v1.6.11.1 annotation composer stable placement jitter");
assertContains(clientSource, "dx < -horizontalThreshold ? \"right\"", "v1.6.11.1 annotation composer puts left targets on right");
assertContains(clientSource, "dy < -verticalThreshold ? \"bottom\"", "v1.6.11.1 annotation composer puts upper targets below");
assertContains(clientSource, "dy > verticalThreshold ? \"bottom\"", "v1.6.11.1 annotation composer puts lower targets below by default");
assertContains(clientSource, "preferred.push(\"bottom\")", "v1.6.11.1 annotation composer puts center targets below by default");
assertContains(clientSource, "lowerTargetGap", "v1.6.11.1 annotation composer keeps lower targets close");
assertContains(styleSource, ".annotation-composer-input.is-multiline", "v1.6.11.1 annotation composer multiline layout");
assertContains(styleSource, "line-height: 32px", "v1.6.11.1 annotation composer single line vertical centering");
assertContains(styleSource, ".codex-inbox-list", "v1.6.11.1 bottom annotation list CSS");
assertContains(styleSource, ".codex-inbox-row p", "v1.6.11.1 bottom annotation body typography");
assertContains(styleSource, ".app-shell[data-theme-mode=\"light\"] .annotation-ask-codex", "v1.6.11.1 light annotation Ask Codex style");
assertContains(styleSource, ".app-shell[data-theme-mode=\"light\"] .annotation-composer-input", "v1.6.11.1 light annotation composer style");
assertContains(styleSource, ".app-shell[data-theme-mode=\"system\"] .annotation-composer-input", "v1.6.11.1 system light annotation composer style");
assertNotContains(styleSource, "#playback-next {\n  border-color: #60a5fa;", "v1.6.11.1 playback next must not be permanently active");
assertNotContains(clientSource, 'data-bottom-tab="notes"', "v1.6.11.1 removes fake notes tab");
assertNotContains(clientSource, "暂无页面备注", "v1.6.11.1 removes fake notes placeholder");
assertContains(clientSource, "previewDescriptors[\"rich-after\"]", "v1.6.11.1 thumbnails fixed to final state");
assertContains(clientSource, "toggleAnchoredPopover", "v1.6.11.1 anchored popover helper");
assertContains(clientSource, "positionPopover", "v1.6.11.1 popover positioning helper");
assertContains(clientSource, "wpsStateShortDescription", "v1.6.11.1 slim WPS state summary");
assertContains(readmeSource, "v1.6.11.1 Playback QA 生产台", "README v1.6 UI section");
assertContains(readmeSource, "真实 PPT 预览", "README v1.6 real PPT preview");
assertContains(readmeSource, "Overlay 开关", "README v1.6 overlay toggles");
assertContains(readmeSource, "播放模式", "README v1.6 playback mode");
assertContains(readmeSource, "鼠标左键", "README v1.6 mouse control");
assertContains(readmeSource, "Space", "README v1.6 Space shortcut");
assertContains(readmeSource, "ArrowRight", "README v1.6 ArrowRight shortcut");
assertContains(readmeSource, "Esc", "README v1.6 Esc shortcut");
assertContains(readmeSource, "动画时间轴", "README v1.6 animation timeline");
assertContains(readmeSource, "底部 tabs", "README v1.6 bottom tabs");
assertContains(readmeSource, "播放 QA 自动记录", "README v1.6 playback QA log");
assertContains(readmeSource, "outputs/playback-qa-log.jsonl", "README v1.6.11.1 playback QA JSONL path");
assertContains(readmeSource, "GET /api/playback-qa", "README v1.6.11.1 playback QA GET API");
assertContains(readmeSource, "POST /api/playback-qa/session", "README v1.6.11.1 playback QA POST API");
assertContains(readmeSource, "specs/presentation-design-system.yaml", "README v1.6.11.1 design system path");
assertContains(readmeSource, "objectId", "README v1.6.11.1 objectId action field");
assertContains(readmeSource, "历史导出", "README v1.6.11.1 historical export status");
assertContains(readmeSource, "Codex 是主生产者", "README v1.6.11.1 Codex-native positioning");
assertContains(readmeSource, "deck-spec / scene beat 驱动", "README v1.6 spec-driven playback note");
assertContains(readmeSource, "不要求解析导出的 PPTX 文件", "README v1.6 no PPTX parsing note");
assertContains(readmeSource, "outputs/revision-plan.yaml", "README revision plan output path");
assertContains(readmeSource, "GET /api/revision-plan", "README revision plan GET API");
assertContains(readmeSource, "POST /api/revision-plan/from-audit", "README revision plan from audit API");
assertContains(readmeSource, "POST /api/revision-plan/action", "README revision plan action API");
assertContains(readmeSource, "source: audit-recommendation", "README audit recommendation source");
assertContains(readmeSource, "source: user-comment", "README user comment source");
assertContains(readmeSource, "不修改 `specs/example.deck-spec.yaml`", "README revision plan does not auto modify spec");
assertContains(readmeSource, "提交后会自动尝试确定性小改", "README v1.6.11.1 automatic deterministic edit");
assertContains(readmeSource, "needs-codex", "README v1.6.11.1 no fake design processing");
assertContains(readmeSource, "events/upload-intent.jsonl", "README v1.6.11.1 upload intent path");
assertContains(readmeSource, "/api/upload-reference/metadata", "README v1.6.11.1 large upload metadata route");
assertContains(readmeSource, "outputs/undo-stack/", "README v1.6.11.1 undo stack path");
assertContains(readmeSource, "outputs/redo-stack/", "README v1.6.11.1 redo stack path");
assertContains(readmeSource, "导出即生成", "README v1.6.11.1 export regenerates");
assertContains(readmeSource, "specs/native-animation-catalog.yaml", "README v1.6.11.1 native animation catalog");
assertContains(readmeSource, "跟随系统", "README theme system mode");
assertContains(readmeSource, "浅色", "README light theme mode");
assertContains(readmeSource, "深色", "README dark theme mode");
assertContains(readmeSource, "项目名修改不再使用浏览器 prompt", "README project name editor fix");
assertContains(readmeSource, "工作流状态条移动到画布上方", "README workflow moved above canvas");
assertContains(readmeSource, "顶部不再常驻展示 PowerPoint / Keynote", "README top compatibility removal");
assertContains(readmeSource, "floating rail「幻灯片」恢复按钮", "README slide restore tab");
assertContains(readmeSource, "floating rail「协作面板」恢复按钮", "README inspector restore tab");
assertContains(readmeSource, "避免冲出边框", "README overflow fix");
assertContains(readmeSource, "severity badge", "README audit severity badge");
assertContains(readmeSource, "交付控制区", "README delivery control area");
assertContains(readmeSource, "动画时间轴", "README animation timeline");
assertContains(readmeSource, "Codex-native 本地 PPTX 生产台", "README Codex-native positioning");
assertContains(readmeSource, "v1.6.11.1 Codex Bridge", "README v1.6.11.1 Codex Bridge section");
assertContains(readmeSource, "events/codex-events.jsonl", "README v1.6.11.1 unified bridge queue");
assertContains(readmeSource, ".codex-bridge/current-thread.json", "README v1.6.11.1 current thread config");
assertContains(readmeSource, "不是网页版 PowerPoint", "README not PowerPoint clone positioning");
assertContains(readmeSource, "/api/workbench-state", "README workbench state API");
assertContains(readmeSource, "outputs/export-history.jsonl", "README export history path");
assertContains(readmeSource, "底部默认只显示轻量状态栏", "README collapsed bottom statusbar");
assertContains(readmeSource, "左侧幻灯片栏可折叠", "README collapsible slide nav");
assertContains(readmeSource, "右侧 inspector 可折叠", "README collapsible inspector");
assertContains(readmeSource, "Rich 点击前 / Rich 点击后 / WPS 初始 / WPS 点击后 / 差异视图", "README v1.2 preview states");
assertContains(readmeSource, "/Users/bruce/Desktop/PPT", "README locked export root");
assertContains(readmeSource, "${projectName}-YYYYMMDD", "README default export folder name");
assertContains(readmeSource, "${projectName}-PowerPoint.pptx", "README PowerPoint filename");
assertContains(readmeSource, "${projectName}-兼容.pptx", "README compatible filename");
assertContains(readmeSource, "(1)", "README duplicate export suffix");
assertContains(readmeSource, "导出文件夹内只包含两个可交付 PPTX", "README export contents");
assertContains(readmeSource, "逐页问题", "README audit page issues");
assertContains(readmeSource, "动画点击风险", "README audit click risks");
assertContains(readmeSource, "WPS 兼容风险", "README audit WPS risks");
assertContains(readmeSource, "revision-plan 草案", "README revision plan draft");
assertContains(readmeSource, "多状态预览只是网页预览模型", "README preview model limitation");
assertContains(readmeSource, "真实验收仍需打开 PPTX", "README real playback validation limitation");
assertContains(readmeSource, "v1.6.11.1 可以把建议写入 `outputs/revision-plan.yaml`", "README revision plan write capability");
assertContains(readmeSource, "不自动改稿、不自动修改 `deck-spec`", "README no auto edit limitation");
